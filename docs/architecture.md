...
worker, separate database/application/egress networks, an environment edge
network, and unique volumes. PostgreSQL is database-network-only; Next has no
database-network access; worker has egress but no edge access. Edge resolves
environment-qualified aliases dynamically through Docker DNS. No service uses
`container_name`; databases, aliases, volumes, S3 namespaces, OAuth/Resend
applications, and secrets are environment-specific.
...
staging-attested manifest and never rebuilds. Database operations use a
database-only Compose contract. First production bootstrap creates only pinned
PostgreSQL and backs up the initial database before migration; later
deployments back up the active database first. Migrations run once before a
bounded health-gated replacement. Only public smoke finalizes current/previous
