...
| Reactions | `test_reaction_catalog.py`, `test_reactions.py`, `test_postgresql_concurrency.py` | Covered: manifest-only catalog import, media normalization/S3 attestation, catalog-ID-only API, legacy Unicode preservation, visibility/tombstones, auth/CSRF, participants cursor, rate limit and parallel toggles | Include the PostgreSQL cases in the explicit no-skip gate |
...
| Local identity and auth email | `tests/test_local_account_api.py`, `tests/test_auth.py`, `tests/test_identity_services.py`, `tests/test_auth_email.py`, `tests/test_stage17_identity_migration.py`, `tests/test_users_postgresql_concurrency.py` | Covered: Unicode nickname contract/history, canonical-email uniqueness, local/OAuth linking, profile completion, CSRF/session rotation, verification/reset/set/change, enumeration resistance, DB rate limits, fragment credentials, durable worker-only delivery, owner/author migration | PostgreSQL concurrency and populated migration chain are mandatory no-skip CI gates; controlled real delivery is staging-only |
